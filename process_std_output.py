...
def create_file_path(c, conn, benchmark):
    ...
    c, conn = process_std_out(c, conn, std_out_directory)
    return c, conn


# process each file
def process_std_out(c, conn, std_out_directory):
    for std_out_file in sorted(os.listdir(std_out_directory)):
        ...
        c.execute('INSERT OR IGNORE INTO StdOutput ' \
                  'VALUES (%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,\'%s\',\'%s\');'
                  % (std_out_file_index, fi_thread_index, fiInstCount, fiDynamicKernelIndex,
                     fiStaticKernelIndex, fiBit, fiBambooIndex, deviceFiThreadIndex, deviceFiInstCount,
                     devicefiBit, original_value, corrupted_value
                     ))
        conn.commit()
    return c, conn
...
# main
def std_out_main(c, conn, benchmark):
